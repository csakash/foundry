...
def check(frames: Sequence[str | Path], max_lum: float, max_rb: float = 12.0,
          box: Box | None = None) -> dict[str, Any]:
    if len(frames) < 2:
    ...
    return result(ok, {"max_lum": round(dl, 1), "max_rb": round(drb, 1), "limit_lum": max_lum,
                  ...
                  "Her skin and light change during the clip; add 'lighting is constant, her face and skin tone stay "
                  "identical throughout' and remove any action that turns her toward the warm lamp.")
